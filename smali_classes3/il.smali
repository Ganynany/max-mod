.class public final synthetic Lil;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements Ljava/lang/Runnable;


# instance fields
.field public final synthetic a:I

.field public final synthetic b:Z

.field public final synthetic c:Ljava/lang/Object;


# direct methods
.method public synthetic constructor <init>(Ljava/lang/Object;ZI)V
    .locals 0

    .line 1
    iput p3, p0, Lil;->a:I

    iput-object p1, p0, Lil;->c:Ljava/lang/Object;

    iput-boolean p2, p0, Lil;->b:Z

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method

.method public synthetic constructor <init>(ZLru/ok/android/externcalls/sdk/audio/internal/impl3/CallsAudioManagerV3Impl;)V
    .locals 1

    .line 2
    const/4 v0, 0x5

    iput v0, p0, Lil;->a:I

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    iput-boolean p1, p0, Lil;->b:Z

    iput-object p2, p0, Lil;->c:Ljava/lang/Object;

    return-void
.end method


# virtual methods
.method public final run()V
    .locals 10

    iget v0, p0, Lil;->a:I

    const/16 v1, 0x17

    const/4 v2, 0x5

    const/4 v3, 0x4

    const/16 v4, 0x8

    const/4 v5, 0x0

    const/4 v6, 0x1

    const/4 v7, 0x0

    packed-switch v0, :pswitch_data_0

    iget-object v0, p0, Lil;->c:Ljava/lang/Object;

    check-cast v0, Lmag;

    iget-boolean v1, p0, Lil;->b:Z

    iget-object v0, v0, Lmag;->b:Ljava/lang/Object;

    check-cast v0, Lm91;

    :try_start_0
    invoke-virtual {v0}, Lm91;->k()Z

    move-result v2
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    iget-object v3, v0, Lm91;->u0:Lreb;

    if-nez v2, :cond_0

    goto/16 :goto_4

    :cond_0
    :try_start_1
    iget-object v2, v0, Lm91;->G0:Lgq1;

    invoke-virtual {v2}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget-boolean v2, v3, Lreb;->b:Z

    if-eqz v2, :cond_1

    iput-boolean v7, v3, Lreb;->b:Z

    iput-boolean v7, v3, Lreb;->c:Z

    invoke-virtual {v3}, Lreb;->a()V

    goto :goto_0

    :cond_1
    move v6, v7

    :goto_0
    if-eqz v6, :cond_2

    invoke-virtual {v0}, Lm91;->z()V

    sget-object v2, Luh1;->o:Luh1;

    invoke-virtual {v0, v2, v5}, Lm91;->m(Luh1;Ljava/lang/Object;)V

    goto :goto_1

    :catchall_0
    move-exception v2

    goto :goto_3

    :cond_2
    :goto_1
    iget-object v2, v0, Lm91;->o0:Ll32;

    if-nez v2, :cond_3

    goto :goto_4

    :cond_3
    iget-boolean v3, v3, Lreb;->b:Z

    if-eqz v3, :cond_4

    iget-boolean v3, v0, Lm91;->Y0:Z

    if-nez v3, :cond_4

    iget-object v3, v0, Lm91;->k1:Lozc;

    goto :goto_2

    :cond_4
    iget-object v3, v0, Lm91;->l1:Lozc;

    :goto_2
    if-nez v3, :cond_5

    goto :goto_4

    :cond_5
    invoke-virtual {v2}, Ll32;->G()Z

    move-result v2

    if-eqz v2, :cond_6

    iget-object v2, v0, Lm91;->o0:Ll32;

    iput-object v3, v2, Ll32;->r:Lozc;

    iget-object v2, v2, Ll32;->m:Llp9;

    if-eqz v2, :cond_8

    iput-object v3, v2, Llp9;->f:Lozc;

    invoke-virtual {v2}, Llp9;->c()V

    goto :goto_4

    :cond_6
    iget-object v2, v0, Lm91;->o0:Ll32;

    iput-object v3, v2, Ll32;->r:Lozc;

    iget-object v4, v2, Ll32;->m:Llp9;

    if-eqz v4, :cond_7

    iput-object v3, v4, Llp9;->f:Lozc;

    invoke-virtual {v4}, Llp9;->c()V

    goto :goto_4

    :cond_7
    invoke-virtual {v2, v3}, Ll32;->U(Lozc;)V
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    goto :goto_4

    :goto_3
    iget-object v0, v0, Lm91;->P:Ljte;

    const-string v3, "Error apply screen capture stopped state (fast="

    const-string v4, ")"

    invoke-static {v3, v4, v1}, Lidg;->t(Ljava/lang/String;Ljava/lang/String;Z)Ljava/lang/String;

    move-result-object v1

    const-string v3, "OKRTCCall"

    invoke-interface {v0, v3, v1, v2}, Ljte;->logException(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)V

    :cond_8
    :goto_4
    return-void

    :pswitch_0
    iget-object v0, p0, Lil;->c:Ljava/lang/Object;

    check-cast v0, Lmvg;

    iget-boolean v1, p0, Lil;->b:Z

    iget-object v0, v0, Lmvg;->n:Ljava/util/concurrent/CopyOnWriteArraySet;

    invoke-virtual {v0}, Ljava/util/concurrent/CopyOnWriteArraySet;->iterator()Ljava/util/Iterator;

    move-result-object v0

    :cond_9
    :goto_5
    invoke-interface {v0}, Ljava/util/Iterator;->hasNext()Z

    move-result v2

    if-eqz v2, :cond_b

    invoke-interface {v0}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Lc91;

    iget-object v2, v2, Lc91;->a:Lk22;

    iget-object v2, v2, Lk22;->k:Lhc4;

    iget-boolean v3, v2, Lhc4;->i:Z

    if-ne v3, v1, :cond_a

    goto :goto_5

    :cond_a
    iput-boolean v1, v2, Lhc4;->i:Z

    iget-object v3, v2, Lhc4;->c:Lgc4;

    iget-boolean v3, v3, Lgc4;->a:Z

    if-eqz v3, :cond_9

    invoke-virtual {v2}, Lhc4;->a()V

    goto :goto_5

    :cond_b
    return-void

    :pswitch_1
    iget-object v0, p0, Lil;->c:Ljava/lang/Object;

    check-cast v0, Lhcd;

    iget-boolean v1, p0, Lil;->b:Z

    iget-object v0, v0, Lhcd;->Q0:Landroid/widget/TextView;

    if-eqz v1, :cond_c

    move v4, v7

    :cond_c
    invoke-virtual {v0, v4}, Landroid/view/View;->setVisibility(I)V

    return-void

    :pswitch_2
    iget-object v0, p0, Lil;->c:Ljava/lang/Object;

    check-cast v0, Lru/ok/messages/media/mediabar/MediaBarPreviewLayout;

    iget-boolean v1, p0, Lil;->b:Z

    iget-object v0, v0, Lru/ok/messages/media/mediabar/MediaBarPreviewLayout;->O0:Landroid/view/View;

    if-eqz v1, :cond_d

    invoke-virtual {v0, v7}, Landroid/view/View;->setVisibility(I)V

    goto :goto_6

    :cond_d
    invoke-virtual {v0, v4}, Landroid/view/View;->setVisibility(I)V

    :goto_6
    return-void

    :pswitch_3
    iget-object v0, p0, Lil;->c:Ljava/lang/Object;

    check-cast v0, Lmb2;

    iget-boolean v1, p0, Lil;->b:Z

    iput-boolean v1, v0, Lmb2;->X0:Z

    if-eqz v1, :cond_f

    iget v1, v0, Lmb2;->c1:I

    if-eq v1, v3, :cond_e

    iget v1, v0, Lmb2;->c1:I

    if-ne v1, v2, :cond_f

    :cond_e
    invoke-virtual {v0, v7}, Lmb2;->J(Z)V

    :cond_f
    return-void

    :pswitch_4
    iget-object v0, p0, Lil;->c:Ljava/lang/Object;

    check-cast v0, Lia2;

    iget-boolean v1, p0, Lil;->b:Z

    iget-boolean v2, v0, Lia2;->a:Z

    if-ne v2, v1, :cond_10

    goto :goto_7

    :cond_10
    iput-boolean v1, v0, Lia2;->a:Z

    if-eqz v1, :cond_11

    iget-boolean v1, v0, Lia2;->b:Z

    if-eqz v1, :cond_12

    iget-object v1, v0, Lia2;->c:Ljava/lang/Object;

    check-cast v1, Lua2;

    invoke-virtual {v1}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    new-instance v2, Loa2;

    invoke-direct {v2, v1, v6}, Loa2;-><init>(Lua2;I)V

    invoke-static {v2}, Lqyk;->c(Lt62;)Lv62;

    move-result-object v1

    invoke-static {v1}, Lzrc;->i(Lp69;)Lp69;

    move-result-object v1

    new-instance v2, Lp3;

    const/16 v3, 0x1c

    invoke-direct {v2, v0, v3}, Lp3;-><init>(Ljava/lang/Object;I)V

    iget-object v3, v0, Lia2;->d:Ljava/lang/Object;

    check-cast v3, Lk7g;

    invoke-interface {v1, v2, v3}, Lp69;->a(Ljava/lang/Runnable;Ljava/util/concurrent/Executor;)V

    iput-boolean v7, v0, Lia2;->b:Z

    goto :goto_7

    :cond_11
    new-instance v1, Landroidx/camera/core/CameraControl$OperationCanceledException;

    const-string v2, "The camera control has became inactive."

    invoke-direct {v1, v2}, Ljava/lang/Exception;-><init>(Ljava/lang/String;)V

    iget-object v2, v0, Lia2;->g:Ljava/lang/Object;

    check-cast v2, Ls62;

    if-eqz v2, :cond_12

    invoke-virtual {v2, v1}, Ls62;->d(Ljava/lang/Throwable;)Z

    iput-object v5, v0, Lia2;->g:Ljava/lang/Object;

    :cond_12
    :goto_7
    return-void

    :pswitch_5
    iget-object v0, p0, Lil;->c:Ljava/lang/Object;

    check-cast v0, Lx82;

    iget-boolean v1, p0, Lil;->b:Z

    invoke-virtual {v0}, Lx82;->e()V

    iget-wide v2, v0, Lx82;->Z:J

    const-wide/16 v4, 0x0

    cmp-long v2, v2, v4

    if-eqz v2, :cond_13

    goto/16 :goto_9

    :cond_13
    iget-object v2, v0, Lx82;->c:Lw82;

    iget-object v2, v2, Lw82;->a:Lzs2;

    iget-wide v2, v2, Lzs2;->a:J

    iget-object v4, v0, Lx82;->d:Ljava/util/concurrent/CopyOnWriteArrayList;

    invoke-virtual {v4}, Ljava/util/concurrent/CopyOnWriteArrayList;->iterator()Ljava/util/Iterator;

    move-result-object v4

    const-wide v5, 0x7fffffffffffffffL

    :cond_14
    :goto_8
    invoke-interface {v4}, Ljava/util/Iterator;->hasNext()Z

    move-result v7

    if-eqz v7, :cond_15

    invoke-interface {v4}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v7

    check-cast v7, Lqha;

    iget-object v7, v7, Lqha;->a:Lhja;

    iget-wide v7, v7, Lhja;->c:J

    cmp-long v9, v7, v5

    if-gez v9, :cond_14

    move-wide v5, v7

    goto :goto_8

    :cond_15
    const-wide/16 v7, 0x1

    sub-long/2addr v5, v7

    new-instance v4, Ljava/lang/StringBuilder;

    const-string v7, "loadNext: from db from: "

    invoke-direct {v4, v7}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-static {v2, v3}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v7

    invoke-static {v7}, Li35;->d0(Ljava/lang/Long;)Ljava/lang/String;

    move-result-object v7

    invoke-virtual {v4, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v7, " to: "

    invoke-virtual {v4, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-static {v5, v6}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v7

    invoke-static {v7}, Li35;->d0(Ljava/lang/Long;)Ljava/lang/String;

    move-result-object v7

    invoke-virtual {v4, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v4}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v4

    const-string v7, "x82"

    invoke-static {v7, v4}, Lgbb;->k(Ljava/lang/String;Ljava/lang/String;)V

    iget-object v4, v0, Lx82;->D0:Lfja;

    invoke-virtual {v4, v2, v3, v5, v6}, Lfja;->i(JJ)Ljava/util/ArrayList;

    move-result-object v2

    iget-object v3, v0, Lx82;->d:Ljava/util/concurrent/CopyOnWriteArrayList;

    invoke-virtual {v3}, Ljava/util/concurrent/CopyOnWriteArrayList;->size()I

    move-result v3

    invoke-virtual {v0, v3, v2}, Lx82;->a(ILjava/util/List;)V

    new-instance v3, Ljava/lang/StringBuilder;

    const-string v4, "loadNext: loaded from db: "

    invoke-direct {v3, v4}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v2}, Ljava/util/ArrayList;->size()I

    move-result v4

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    const-string v4, " messages"

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v3

    invoke-static {v7, v3}, Lgbb;->k(Ljava/lang/String;Ljava/lang/String;)V

    invoke-virtual {v2}, Ljava/util/ArrayList;->isEmpty()Z

    move-result v3

    iput-boolean v3, v0, Lx82;->b:Z

    invoke-virtual {v0}, Lx82;->f()V

    if-eqz v1, :cond_16

    invoke-virtual {v2}, Ljava/util/ArrayList;->size()I

    move-result v1

    const/16 v2, 0x64

    if-ge v1, v2, :cond_16

    iget-object v1, v0, Lx82;->c:Lw82;

    iget-boolean v1, v1, Lw82;->d:Z

    if-eqz v1, :cond_16

    invoke-virtual {v0}, Lx82;->d()V

    :cond_16
    :goto_9
    return-void

    :pswitch_6
    iget-boolean v0, p0, Lil;->b:Z

    iget-object v1, p0, Lil;->c:Ljava/lang/Object;

    check-cast v1, Lru/ok/android/externcalls/sdk/audio/internal/impl3/CallsAudioManagerV3Impl;

    invoke-static {v0, v1}, Lru/ok/android/externcalls/sdk/audio/internal/impl3/CallsAudioManagerV3Impl;->b(ZLru/ok/android/externcalls/sdk/audio/internal/impl3/CallsAudioManagerV3Impl;)V

    return-void

    :pswitch_7
    iget-object v0, p0, Lil;->c:Ljava/lang/Object;

    check-cast v0, Liif;

    iget-boolean v1, p0, Lil;->b:Z

    iget-object v0, v0, Liif;->a:Ljava/lang/Object;

    check-cast v0, Ldd0;

    iput-boolean v1, v0, Ldd0;->q:Z

    iget v1, v0, Ldd0;->g:I

    const/4 v2, 0x2

    if-ne v1, v2, :cond_17

    invoke-virtual {v0}, Ldd0;->a()V

    :cond_17
    return-void

    :pswitch_8
    iget-object v0, p0, Lil;->c:Ljava/lang/Object;

    check-cast v0, Lqia;

    iget-boolean v1, p0, Lil;->b:Z

    iget-object v0, v0, Lqia;->c:Ljava/lang/Object;

    check-cast v0, Lh4f;

    iget-boolean v2, v0, Lh4f;->a0:Z

    if-eq v2, v1, :cond_18

    iput-boolean v1, v0, Lh4f;->a0:Z

    invoke-virtual {v0, v6}, Lh4f;->K(Z)V

    goto :goto_a

    :cond_18
    new-instance v0, Ljava/lang/StringBuilder;

    const-string v2, "Audio source silenced transitions to the same state "

    invoke-direct {v0, v2}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Z)Ljava/lang/StringBuilder;

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    const-string v1, "Recorder"

    invoke-static {v1, v0}, Lfte;->g(Ljava/lang/String;Ljava/lang/String;)V

    :goto_a
    return-void

    :pswitch_9
    iget-object v0, p0, Lil;->c:Ljava/lang/Object;

    check-cast v0, Lwy9;

    iget-boolean v3, p0, Lil;->b:Z

    iget-object v0, v0, Lwy9;->c:Ljava/lang/Object;

    check-cast v0, Lka6;

    sget v4, Ltyi;->a:I

    iget-object v0, v0, Lka6;->a:Lqa6;

    iget-boolean v4, v0, Lqa6;->k1:Z

    if-ne v4, v3, :cond_19

    goto :goto_b

    :cond_19
    iput-boolean v3, v0, Lqa6;->k1:Z

    iget-object v0, v0, Lqa6;->C0:Lx3b;

    new-instance v4, Lcs2;

    invoke-direct {v4, v3, v2}, Lcs2;-><init>(ZI)V

    invoke-virtual {v0, v1, v4}, Lx3b;->k(ILy69;)V

    :goto_b
    return-void

    :pswitch_a
    iget-object v0, p0, Lil;->c:Ljava/lang/Object;

    check-cast v0, Lyc0;

    iget-boolean v2, p0, Lil;->b:Z

    iget-object v0, v0, Lyc0;->b:Lla6;

    sget-object v4, Lvyi;->a:Ljava/lang/String;

    iget-object v0, v0, Lla6;->a:Lra6;

    iget-boolean v4, v0, Lra6;->q1:Z

    if-ne v4, v2, :cond_1a

    goto :goto_c

    :cond_1a
    iput-boolean v2, v0, Lra6;->q1:Z

    iget-object v0, v0, Lra6;->E0:Le79;

    new-instance v4, Lcs2;

    invoke-direct {v4, v2, v3}, Lcs2;-><init>(ZI)V

    invoke-virtual {v0, v1, v4}, Le79;->f(ILz69;)V

    :goto_c
    return-void

    :pswitch_b
    iget-object v0, p0, Lil;->c:Ljava/lang/Object;

    check-cast v0, Lwo7;

    iget-boolean v1, p0, Lil;->b:Z

    iget-object v2, v0, Lwo7;->c:Ljava/lang/Object;

    check-cast v2, Lsqg;

    iget-object v2, v2, Lsqg;->d:Lorg/webrtc/PeerConnectionFactory;

    iget-object v3, v0, Lwo7;->a:Ljava/lang/Object;

    check-cast v3, Ljava/lang/String;

    iget-object v0, v0, Lwo7;->X:Ljava/lang/Object;

    check-cast v0, Lorg/webrtc/NativeDoubleArrayConsumer$Consumer;

    invoke-virtual {v2, v1, v3, v0}, Lorg/webrtc/PeerConnectionFactory;->setAnimojiParams(ZLjava/lang/String;Lorg/webrtc/NativeDoubleArrayConsumer$Consumer;)V

    return-void

    nop

    :pswitch_data_0
    .packed-switch 0x0
        :pswitch_b
        :pswitch_a
        :pswitch_9
        :pswitch_8
        :pswitch_7
        :pswitch_6
        :pswitch_5
        :pswitch_4
        :pswitch_3
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch
.end method
