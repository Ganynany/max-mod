.class public final Lijg;
.super Lxwg;
.source "SourceFile"


# instance fields
.field public final o:Lhjg;


# direct methods
.method public constructor <init>(Lhjg;Ljava/util/concurrent/ExecutorService;)V
    .locals 0

    invoke-direct {p0, p2}, Lxwg;-><init>(Ljava/util/concurrent/Executor;)V

    iput-object p1, p0, Lijg;->o:Lhjg;

    return-void
.end method


# virtual methods
.method public final bridge synthetic L(Lfyg;I)V
    .locals 0

    check-cast p1, Ltjg;

    invoke-virtual {p0, p1, p2}, Lijg;->N(Ltjg;I)V

    return-void
.end method

.method public final N(Ltjg;I)V
    .locals 4

    instance-of v0, p1, Lrjg;

    if-eqz v0, :cond_1

    check-cast p1, Lrjg;

    iget-object v0, p1, Lw5f;->a:Landroid/view/View;

    invoke-virtual {p0, p2}, Lt59;->G(I)Ljava/lang/Object;

    move-result-object p2

    check-cast p2, Lb69;

    check-cast p2, Lgjg;

    move-object v1, v0

    check-cast v1, Lqjg;

    invoke-virtual {v1, p2}, Lqjg;->setModelItem(Lgjg;)V

    iget-object v1, p0, Lijg;->o:Lhjg;

    iput-object v1, p1, Lrjg;->L0:Lhjg;

    invoke-interface {p2}, Lgjg;->e()Ldjg;

    move-result-object p1

    instance-of p1, p1, Lbjg;

    if-eqz p1, :cond_0

    move-object p1, v0

    check-cast p1, Lqjg;

    new-instance v2, Lkn;

    const/16 v3, 0xe

    invoke-direct {v2, v1, v3}, Lkn;-><init>(Ljava/lang/Object;I)V

    invoke-virtual {p1, v2}, Lqjg;->setOnSwitchCheckedListener(Lff7;)V

    goto :goto_0

    :cond_0
    move-object p1, v0

    check-cast p1, Lqjg;

    const/4 v2, 0x0

    invoke-virtual {p1, v2}, Lqjg;->setOnSwitchListener(Lmjg;)V

    :goto_0
    new-instance p1, Lo8d;

    const/16 v2, 0xe

    invoke-direct {p1, v1, v2, p2}, Lo8d;-><init>(Ljava/lang/Object;ILjava/lang/Object;)V

    invoke-static {v0, p1}, Lld7;->N(Landroid/view/View;Landroid/view/View$OnClickListener;)V

    return-void

    :cond_1
    instance-of v0, p1, Lnmg;

    if-eqz v0, :cond_2

    invoke-virtual {p0, p2}, Lt59;->G(I)Ljava/lang/Object;

    move-result-object p2

    check-cast p2, Lb69;

    invoke-virtual {p1, p2}, Lfyg;->C(Lb69;)V

    :cond_2
    return-void
.end method

.method public final bridge synthetic v(Lw5f;I)V
    .locals 0

    check-cast p1, Ltjg;

    invoke-virtual {p0, p1, p2}, Lijg;->N(Ltjg;I)V

    return-void
.end method

.method public final w(Lw5f;ILjava/util/List;)V
    .locals 3

    check-cast p1, Ltjg;

    invoke-interface {p3}, Ljava/util/Collection;->isEmpty()Z

    move-result v0

    if-nez v0, :cond_3

    new-instance v0, Lejg;

    const/4 v1, 0x5

    invoke-direct {v0, v1}, Lrr0;-><init>(I)V

    invoke-interface {p3}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object p3

    :cond_0
    :goto_0
    invoke-interface {p3}, Ljava/util/Iterator;->hasNext()Z

    move-result v1

    if-eqz v1, :cond_2

    invoke-interface {p3}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v1

    instance-of v2, v1, Lejg;

    if-eqz v2, :cond_1

    check-cast v1, Lejg;

    goto :goto_1

    :cond_1
    const/4 v1, 0x0

    :goto_1
    if-eqz v1, :cond_0

    invoke-virtual {v0, v1}, Lrr0;->C(Lrr0;)V

    goto :goto_0

    :cond_2
    iget-object p3, p0, Lt59;->d:Lq10;

    iget-object p3, p3, Lq10;->f:Ljava/util/List;

    invoke-interface {p3, p2}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object p2

    check-cast p2, Lb69;

    invoke-virtual {p1, p2, v0}, Lfyg;->D(Lb69;Ljava/lang/Object;)V

    return-void

    :cond_3
    invoke-virtual {p0, p1, p2}, Lijg;->N(Ltjg;I)V

    return-void
.end method

.method public final x(Landroid/view/ViewGroup;I)Lw5f;
    .locals 2

    sget v0, Lphc;->g:I

    if-ne p2, v0, :cond_0

    new-instance p2, Lnmg;

    invoke-virtual {p1}, Landroid/view/View;->getContext()Landroid/content/Context;

    move-result-object p1

    new-instance v0, Lmmg;

    invoke-direct {v0, p1}, Lmmg;-><init>(Landroid/content/Context;)V

    invoke-direct {p2, v0}, Lw5f;-><init>(Landroid/view/View;)V

    return-object p2

    :cond_0
    new-instance p2, Lrjg;

    invoke-virtual {p1}, Landroid/view/View;->getContext()Landroid/content/Context;

    move-result-object p1

    new-instance v0, Lqjg;

    const/4 v1, 0x0

    invoke-direct {v0, p1, v1}, Lqjg;-><init>(Landroid/content/Context;I)V

    invoke-direct {p2, v0}, Lw5f;-><init>(Landroid/view/View;)V

    return-object p2
.end method
